...
文章正文、封面、关联相册和列表展示选项属于草稿，发布后复制为独立不可变版本；素材引用属于对应版本。站点关于选篇使用独立 about_article_version，避免与名称／联系链接保存互相覆盖。所有下线与选篇操作串行锁定站点；正在用于关于的文章不能下线。

前后台均设置 `GALLERY_ARTICLE_MEDIA_ROOT` 为同一个绝对目录，本地采用 `.gallery-local/article-media`。Compose 使用 `GALLERY_ARTICLE_MEDIA_PATH` 绑定至 `/media/articles`，前台只读、后台可写；预先创建目录并给镜像 node 用户（UID 1000）适当读写权限，不使用全局可写权限。后台同时设置 `BODY_SIZE_LIMIT=12M`；接口仍限制每张图片 10 MiB，仅解码 JPEG／PNG／WebP 单图，最多 4000 万输入像素。生成 1600／3840 最大边的 WebP 展示文件并移除元数据，不保存上传原件；Immich 摄影照片仍引用原有 1080p／4K 衍生图。

上传先写 `.upload-*` 临时目录，准备好两个文件后原子重命名到随机 storage_key 目录，再插入可用素材行。失败会清理暂存文件；进程被强制终止时可能留下无数据库引用的暂存或孤立目录。只有后台认证接口可读取未公开素材。匿名媒体 URL 必须包含文章 ID，且素材属于该文章的当前公开版本；旧版本和已下线文章不授权。公开响应 no-store。

后台素材选择框可按名称检索、分页、查看引用文章数量并删除未引用素材；曾进入发布版本的引用保留用于审计，因此即使旧版本不再公开，也不会直接删除其文件。没有自动清理任务。若需处理异常残留，先停后台写入并备份，核对目录名与 article_media.storage_key；仅移除至少 24 小时且数据库中无记录的 `.upload-*`／孤立 UUID 目录，不删除任何有数据库记录的文件夹。

备份必须成对覆盖：完整 PostgreSQL dump、文章素材目录（保留目录名及文件权限）、独立运行配置。短暂停止 Gallery 后台写入，完成 dump 与素材目录快照后再恢复后台；不要在备份时单独删除或迁移素材。恢复首先使用隔离库与复制出的素材根目录，比较文章／版本／引用数及正文摘要，并使用 gallery_public 实测公开素材、下线后的拒绝和关于选篇。旧会话一律撤销。隔离验收已增加数据库恢复与素材目录复制验证；实际结果见本轮交付记录。
