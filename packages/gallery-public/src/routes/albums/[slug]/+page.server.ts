import { publicCatalog, relatedArticles } from '@gallery/db/server';
...
export async function load({ params, url }: { params: { slug: string }; url: URL }) {
  try {
    ...
    return {
      ...catalog,
      relatedArticles: catalog.active ? await relatedArticles(app.db, catalog.active.id) : [],
      origin: app.origin,
      initialPage: Math.max(
        1,
        Math.min(100000, Number.parseInt(url.searchParams.get('page') ?? '1', 10) || 1),
      ),
    };
  } catch (e) {
