...
export async function assertDatabaseCompatibility(db: Kysely<unknown>, service: DatabaseService): Promise<void> {
  ...
  await sql`SELECT album_id, photo_id, asset_id, location_mode, latitude, longitude, group_id, description_format, taken_at, first_added_at FROM gallery.published_photo LIMIT 0`.execute(
    db,
  );
